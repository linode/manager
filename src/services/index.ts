...
export const transformIntoCancellableRequest = <T>(config: any, promiseReject: any): CancellableRequest<T> => {
  const source = Axios.CancelToken.source();
  if (!!promiseReject) {
    return {
      cancel: () => null,
...
export const generateRequest =
  <T>(isCancellable: boolean, ...fns: Function[]): CancellableRequest<T> => {
    const config = reduceRequestConfig(...fns);

    let promiseReject: (() => Promise<never>) | null = null;
    ...
    /*
     * If the request has been marked cancellable by the first arg
     * of this function, return a cancelToken so that we can
     * cancel the request in the consumer 
     */
    if (isCancellable) {
      return transformIntoCancellableRequest(config, promiseReject)
    }
    /*
     * Otherwise, just return the request as normal. cancel() doing nothing
     */
    return {
      cancel: () => null,
