...
export type EntityType = 'linode' | 'nodebalancer' | 'domain' | 'image' | 'volume';
 ...
 */
const _getEntityByIDFromStore = (entityType: EntityType, entityID: string | number) => {
  if (!entityType || !entityID) { return false; }
  const _store = store.getState();
  const { linodes, domains, nodeBalancers, images, volumes } = _store.__resources;
  switch (entityType) {
    case 'linode':
