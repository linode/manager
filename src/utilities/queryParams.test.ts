import { getParamFromUrl, getParamsFromUrl, getQueryParam } from './queryParams';

describe("Url/query parsing utilities", () => {
  describe("getParamsFromUrl function", () => {
    it("should parse a url", () => {
      expect(getParamsFromUrl('https://example.com/?query=false')).toEqual({ query: 'false'});
    });
    it("should handle multiple key/value pairs", () => {
      expect(getParamsFromUrl('https://example.com/?query=false&this=that')).toEqual({ query: 'false', this: 'that'});
    });
    it("should handle escaped whitespace", () => {
      expect(getParamsFromUrl('https://example.com/?query=this%20that')).toEqual({ query: 'this that'});
    });
  ...
  describe("getQueryParam function", () => {
    ...
    it("should return the default value if no value is present", () => {
      expect(getQueryParam('?query=','notaquery','defaultQuery')).toEqual('defaultQuery');
    });
  ...
  describe("getParamFromUrl method", () => {
    it("should return a single query param from a URL string", () => {
      expect(getParamFromUrl('https://example.com/?query=false&this=that', 'this')).toBe('that');
    });
    it("should return the default value if no value is present", () => {
      expect(getQueryParam('https://example.com/?query=','notaquery','defaultQuery')).toEqual('defaultQuery');
    });
  });
})
