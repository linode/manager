...
const actionTypeGenerator = (s: string) =>
  `@manager/features/linodeDetail/disks/${s}`;
...
// STATE
export type State = RequestableData<Linode.Disk[]>;
...
export const handleSuccess = (payload: Linode.Disk[]) => ({
  type: SUCCESS,
  payload
});
...
export const handleUpdate = (
  updateFn: (v: Linode.Disk[]) => Linode.Disk[]
...
// ASYNC
export const _getLinodeDisks: ThunkActionCreator<void> = (linodeId: number) => (
  dispatch,
  getState
) => {
  dispatch(load());
