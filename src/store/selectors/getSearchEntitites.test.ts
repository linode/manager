...
import { domains, images, linodes, nodeBalancers, types, volumes } from 'src/__data__';
...
describe("getSearchEntities selector", () => {
  const mockState: any = {
    ...
    volumes: { itemsById: volumes.reduce((result, c) => ({ ...result, [c.id]: c }), {}) },
    nodeBalancers: { itemsById: nodeBalancers.reduce((result, c) => ({ ...result, [c.id]: c }), {}) },
  }
