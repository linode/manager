...
import { addIconsToAttachments, TicketAttachmentList } from './TicketAttachmentList';

const props = {
  attachments: ['file1.png', 'file2.jpg', 'file3.tiff',
    'file4.bmp', 'file5.jpeg', 'file6.txt', 'file7.pdf'],
...
describe("TicketAttachmentList component", () => {
  ...
  it("should use Show More Files when showMoreAttachments is false", () => {
    expect(component.find('WithStyles(ShowMoreExpansion)').first().props()).toHaveProperty('name', 'Show More Files');
  });
  it("should use Show Less Files when showMoreFiles is true", () => {
    component.setProps({showMoreAttachments: true});
    expect(component.find('WithStyles(ShowMoreExpansion)').first().props()).toHaveProperty('name', 'Show Less Files');
  });
