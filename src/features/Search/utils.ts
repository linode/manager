...
export const searchVolumes = (volumes: Linode.Volume[], query: string) =>
  volumes
    .filter(volume => filterMatched(query, volume.label, volume.tags))
    .map(volume => ({
      label: volume.label,
      value: volume.id,
      data: {
...
export const searchNodeBalancers = (
  nodebalancers: Linode.NodeBalancer[],
  query: string
) =>
  nodebalancers
    .filter(nodebal => filterMatched(query, nodebal.label, nodebal.tags || []))
    .map(nodebal => ({
      label: nodebal.label,
      value: nodebal.id,
      data: {
...
export const searchDomains = (domains: Linode.Domain[], query: string) =>
  domains
    .filter(domain => filterMatched(query, domain.domain, domain.tags))
    .map(domain => ({
      label: domain.domain,
      value: domain.id,
      data: {
