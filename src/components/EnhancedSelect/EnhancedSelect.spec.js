...
describe('Enhanced Select -', () => {
    ...
    describe('Basic Select -', () => {
        ...
        it('should display the placeholder text in the select', () => {
            ...
            expect(basicSelects[0].getText()).withContext(`Select Title should be Basic Select`).toContain('Basic Select');
            expect(basicSelects[0].getText()).withContext(`Placeholder message should be ${placeholderMsg}`)
        ...
        it('should display options on click', () => {
            ...
            expect(options.length).withContext(`There should be 5 select options`).toBe(5);
        });
    ...
    describe('Basic Select with Error - ', () => {
        it('should display the error', () => {
            ...
            expect(error.length).withContext(`Should only be one <p> with error text`).toBe(1);
            expect(error[0].isDisplayed()).withContext(`The error message should be displayed`).toBe(true);
        });
    });
    ...
    describe('Multi Select -', () => {
        ...
        it('should display the multi select with placeholder text', () => {
            ...
            expect(multiSelect.getText()).withContext(`Placeholder text should be ${multiString}`).toBe(`${multiString}`);
        });
        ...
        it('should remove the chip from the select options', () => {
            ...
            expect(remainingOptions.length).withContext(`Menu options should be reduced to 4`).toBe(4);
            expect(remainingOptions).withContext(`Menu options should no longer include selected ${selectedOption}`)
            .not.toContain(selectedOption);
        });

        it('should remove the chip on click of the remove icon', () => {
            $('#multi-select svg').click()
            expect(selectedOption = options[0].getText()).withContext(`Apple should be the first select option`).toBe('Apple');
            expect(multiSelect.getText()).withContext(`Default select text should be ${multiString}`).toBe(`${multiString}`);
        });
    });
    ...
    describe('Creatable Select -', () => {
        ...
        it('should display the creatable select with placeholder text', () => {
            ...
            expect(creatableSelect.isDisplayed()).withContext(`Creatable Select text should be displayed`).toBe(true);
            expect(creatableSelect.getText()).withContext(`Placeholder text should be ${createdText}`).toBe(`${createdText}`);
        });

        it('should display the create menu option', () => {
            newOption = 'foo';
            creatableSelect.$('..').$('input').setValue(newOption);
            expect($(`[data-qa-option="${newOption}"]`).isDisplayed()).withContext(`Select option of ${newOption} should be displayed`).toBe(true);
            expect($(`[data-qa-option="${newOption}"]`).getText()).withContext(`Selected option value should be ${newOption}`).toBe(`Create "${newOption}"`);
        });
        ...
        it('should remove the created option on click of remove icon', () => {
            ...
            .withContext(`${newOption} should not be in the select list`).toBe(true)
        });
    });
});
