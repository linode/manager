...
describe('BarPercent', () => {
  ...
  isFetchingValue prop is true and loadingText prop is not undefined`, () => {
      ...
      expect(componentLoading.find('WithStyles(LinearProgress)').prop('variant')).toBe('indeterminate')
    });
  ...
  it('should return determinate progress bar if isFetchingValue prop is false', () => {
    ...
    expect(componentLoading.find('WithStyles(LinearProgress)').prop('variant')).toBe('determinate')
  });
})