...
describe('linodes/layouts/LinodeGeneral', () => {
  ...
  it('renders link to networking', async () => {
    ...
    expect(page.find('.row').at(0).find('a').at(0).text()).to.equal('(...)');
  });
  it('renders backups not enabled', async () => {
    ...
    expect(page.contains(<div className="col-sm-8 right">Backups not enabled.</div>)).to.equal(true);
  });
  ...
  it('renders plan', async () => {
    ...
    expect(page.contains(<div className="col-sm-8 right">{testLinode.services.linode}</div>)).to.equal(true);
  });
  it('renders datacenter', async () => {
    ...
    expect(page.contains(<div className="col-sm-8 right">{testLinode.datacenter.label}</div>)).to.equal(true);
  });
  it('renders distribution', async () => {
    ...
    expect(page.contains(<div className="col-sm-8 right">{testLinode.distribution.label}</div>)).to.equal(true);
  });
  ...
  it('renders lish path', async () => {
    ...
    expect(page.find('.input-group').at(1).contains(
      <input type="text" className="form-control"
        value={`ssh -t caker@lish-${testLinode.datacenter.datacenter}.linode.com`} 
        readOnly/>)
    ).to.equal(true);
  });
