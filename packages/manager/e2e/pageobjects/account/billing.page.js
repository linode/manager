...
class Billing extends Page {
  ...
  updateElemsDisplay() {
    ...
    expect(this.updateFirstName.isDisplayed())
      .withContext(`"${this.updateFirstName.selector}" selector ${assertLog.displayed}`)
      .toBe(true);
    expect(this.updateLastName.isDisplayed())
      .withContext(`"${this.updateFirstName.selector}" selector ${assertLog.displayed}`)
      .toBe(true);
    expect(this.updateAddress1.isDisplayed())
      .withContext(`"${this.updateLastName.selector}" selector ${assertLog.displayed}`)
      .toBe(true);
    expect(this.updateAddress2.isDisplayed())
      .withContext(`"${this.updateAddress2.selector}" selector ${assertLog.displayed}`)
      .toBe(true);
    expect(this.updateEmail.isDisplayed())
      .withContext(`"${this.updateEmail.selector}" selector ${assertLog.displayed}`)
      .toBe(true);
    expect(this.updatePhone.isDisplayed())
      .withContext(`"${this.updatePhone.selector}" selector ${assertLog.displayed}`)
      .toBe(true);
    expect(this.updateCity.isDisplayed())
      .withContext(`"${this.updateCity.selector}" selector ${assertLog.displayed}`)
      .toBe(true);
    expect(this.updateStateProvince.isDisplayed())
      withContext(`"${this.updateStateProvince.selector}" selector ${assertLog.displayed}`)
      .toBe(true);
    expect(this.updatePostCode.isDisplayed())
      .withContext(`"${this.updatePostCode.selector}" selector ${assertLog.displayed}`)
      .toBe(true);
    expect(this.updateTaxId.isDisplayed())
      .withContext(`"${this.updateTaxId.selector}" selector ${assertLog.displayed}`)
      .toBe(true);
    expect(this.updateButton.isDisplayed())
      withContext(`"${this.updateButton.selector}" selector ${assertLog.displayed}`)
      .toBe(true);
    expect(this.updateButton.getTagName())
      .withContext(`${assertLog.incorrectText} for "${this.resetButton.selector}" selector`)
      .toBe('button');
    expect(this.resetButton.isDisplayed())
      .withContext(`"${this.resetButton.selector}" selector ${assertLog.displayed}`)
      .toBe(true);
  ...
  invoicesDisplay() {
    ...
    $$(this.invoice.selector).forEach(inv => {
      ...
      expect(inv.$(this.dateCreated.selector).getText())
        .withContext(`${assertLog.incorrectRegExVal} "${this.dateCreated.selector}" selector`)
        .toMatch(dateRegex);
      expect(inv.$(this.description.selector).getText())
        .withContect(`${assertLog.incorrectRegExVal} "${this.dateCreated.selector}" selector`)
        .toMatch(/\d/);
