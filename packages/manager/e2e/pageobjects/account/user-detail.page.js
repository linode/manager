...
class UserDetail extends Page {
  ...
  baseElementsDisplay(owner) {
    ...
    expect(this.deleteSubHeader.isDisplayed())
      .withContext(`"${this.deleteSubHeader.selector}" selector ${assertLog.displayed}`)
      .toBe(true);
    ...
    expect(this.usernameField.isDisplayed())
      .withContext(`"${this.usernameField.selector}" selector ${assertLog.displayed}`)
      .toBe(true);

    expect(this.emailWarningToolTip.isDisplayed())
      .withContext(`"${this.emailWarningToolTip.selector}" selector ${assertLog.displayed}`)
      .toBe(true);
    expect(this.saveButton.isDisplayed())
      .withContext(`"${this.saveButton.selector}" selector ${assertLog.displayed}`)
      .toBe(true);
  }
