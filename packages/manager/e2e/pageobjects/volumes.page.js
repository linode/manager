...
class Volumes extends Page {
  ...
  removeVolume(volumeElement) {
    ...
    expect(dialogTitle.isDisplayed())
      .withContext(`"${dialogTitle.selector} selector ${assertLog.displayed}`)
      .toBe(true);
    expect(dialogTitle.getText())
      .withContext(`"${dialogTitle.selector} selector ${assertLog.displayed}`)
      .toMatch('Delete');
    expect(dialogConfirm.isDisplayed())
      .withContext(`"${dialogConfirm.selector} selector ${assertLog.displayed}`)
      .toBe(true);
    expect(dialogConfirm.getTagName())
      .withContext(
        `${assertLog.incorrectTagName} "${dialogConfirm.selector} selector`
      )
      .toBe('button');
    expect(dialogCancel.isDisplayed())
      .withContext(`"${dialogCancel.selector} selector ${assertLog.displayed}`)
      .toBe(true);
    expect(dialogCancel.getTagName())
      .withContext(
        `${assertLog.incorrectTagName} "${dialogCancel.selector} selector`
      )
      .toBe('button');
  ...
  detachConfirm(linodeLabel) {
    ...
    if (!browser.getUrl().includes('/linodes')) {
      $(`[data-qa-volume-cell-attachment="${linodeLabel}"]`).waitForDisplayed(
  ...
  isAttached(volumeElement) {
    const attached =
      volumeElement.$(this.volumeAttachment.selector).getText() !== ''
        ? true
        : false;
    return attached;
  }
