...
const CLEAR_ICON_TESTID = 'clear-icon';
...
const addFilter = ({ index, dimension, operator, value }: AddFilterProps) => {
  ...
  cy.get(`[data-testid="dimension_filters.${index}-id"]`).within(() => {
    ui.autocomplete.findByLabel('Dimension').should(VISIBLE).type(dimension);

    ui.autocompletePopper.findByTitle(dimension).should(VISIBLE).click();

    ui.autocomplete.findByLabel('Operator').should(VISIBLE).type(operator);

    ui.autocompletePopper.findByTitle(operator).should(VISIBLE).click();
...
describe('Cloud pulse widget level dimension filter ', () => {
  ...
  it('should verify deleting a filter from the filter drawer', () => {
    ...
    // Delete the filter
    ui.button
      .findByAttribute('data-testid', CLEAR_ICON_TESTID)
      .should(VISIBLE)
      .and(ENABLED)
      .first()
  ...
  it('should verify that widget level filters exclude global dashboard filters for all widgets', () => {
    ...
    dashboard.widgets.forEach((widget, index) => {
      ...
      cy.get('[data-testid="dimension_filters.0-id"]').within(() => {
        ui.autocomplete.findByLabel('Dimension').should(VISIBLE).click();
      });
