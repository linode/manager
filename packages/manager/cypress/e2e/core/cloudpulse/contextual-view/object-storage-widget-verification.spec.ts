...
import { mockGetBucketObjects } from 'support/intercepts/object-storage';
...
describe('Integration Tests for Object Storage Dashboard - Group By and Widget Verification', () => {
  ...
  beforeEach(() => {
    ...
    mockGetBucketsForRegion(mockRegion.id, [bucketMock[0]]).as(
      'getBucketsForRegion'
    );
  ...
  it('should apply Group By at the dashboard level and verify the metrics API calls', () => {
    ...
    cy.get('[data-testid="drawer"]')
      .find('p')
      .first()
      .and('have.text', 'Object Storage Dashboard');
