...
const regions = [
  regionFactory.build({
    ...
    id: 'us-ord',
    label: 'Chicago, IL',
    monitors: { alerts: ['Object Storage'] },
  }),
  regionFactory.build({
    capabilities: ['Object Storage'],
    country: 'us',
...
describe('Integration Tests for Alert Show Detail Page', () => {
  ...
  entityGroupingOptions.forEach(({ label: groupLabel, value }) => {
    it(`should correctly display the details of the objectstorage alert in the alert details view for ${groupLabel} level`, () => {
      ...
      mockGetCloudPulseServiceByType('objectstorage', services);
      ...
      cy.get('[data-qa-section="Overview"]').within(() => {
        ...
        cy.findByText('objectstorage').should('be.visible');
