...
// We currently have two events menu implementations: our legacy implementation,
// and our upcoming refactor coined `eventMessagesV2`.
// We need to test both implementations since our legacy implementation is
// currently used in Production, but will be switched to the new implementation
// in the near future.
...
describe('Events menu', () => {
  // Parameterize our tests against the `testParameters` array.
  // This allows the tests to run against both event messages implementations.
  testParameters.forEach((testParameter) => {
    describe(testParameter.label, () => {
      // Apply feature flag mocks that correspond to the parameterized environment
      // that we're testing.
      beforeEach(() => {
        testParameter.hook();
      });
      ...
      it('Shows notice when there are no recent events', () => {
        ...
        cy.get('[data-qa-notification-menu]')
          ...
          .within(() => {
            ...
            cy.findByText(/No recent events to display\.?/).should(
              'be.visible'
            );
          });
      });
