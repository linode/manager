    ...
    function(yargs) {
      return yargs
        .option('r', {
          alias: 'record',
          describe:
            'Record Linode API calls by creating an imposter that proxies to a given environment'
        })
        .option('s', {
          alias: 'save',
          describe:
            'Save the current session API calls to the path specified. Configs are JSON files saved to `src/__data__/mocks`'
        })
        .option('l', {
