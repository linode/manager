...
describe('handleAPIErrors', () => {
  it('should handle api error with a field', () => {
    handleAPIErrors(
      (errorWithField as unknown) as APIError[],
      setFieldError,
      setError
    );
  ...
  it('should handle a general api error', () => {
    handleAPIErrors(
      (errorWithoutField as unknown) as APIError[],
      setFieldError,
      setError
    );
...
describe('handleVpcAndConvertSubnetErrors', () => {
  it('converts API errors for subnets into a map of subnet index to SubnetErrors', () => {
    const errors = handleVPCAndSubnetErrors(
      (subnetErrors as unknown) as APIError[],
      setFieldError,
      setError
    );
  ...
  it('takes the last error to display if a subnet field has multiple errors associated with it', () => {
    const errors = handleVPCAndSubnetErrors(
      (subnetMultipleErrorsPerField as unknown) as APIError[],
      setFieldError,
      setError
    );
...
describe('getFormikErrorsFromAPIErrors', () => {
  it('should convert APIError[] to errors in the shape formik expects', () => {
    ...
    for (const { apiErrors, expected } of testCases) {
      expect(
        getFormikErrorsFromAPIErrors((apiErrors as unknown) as APIError[])
      ).toEqual(expected);
    }
  });
});
