...
/**
 * Module-level storage for the EUUID.
 * Set by the API response interceptor when X-Customer-Uuid header is present.
 */
let storedCustomerUuid: null | string = null;

/**
 * Stores the EUUID for use in page view events.
 * Called by the API response interceptor.
 */
export const setStoredCustomerUuid = (euuid: string): void => {
  storedCustomerUuid = euuid;
};

/**
 * Retrieves the stored EUUID for inclusion in page view events.
 * Returns null if EUUID hasn't been captured yet.
 */
export const getStoredCustomerUuid = (): null | string => {
  return storedCustomerUuid;
};

/**
 * Sends the EUUID (Enterprise UUID) to Adobe Analytics via a direct call rule.
 * This is called from the API response interceptor when the X-Customer-Uuid header is present.
 * The EUUID is used to identify customers, including those with restricted billing access.
 *
 * @param euuid - The Enterprise UUID from the X-Customer-Uuid HTTP header
 */
export const sendCustomerUuidEvent = (euuid: string): void => {
  // Store the EUUID for inclusion in page view events
  setStoredCustomerUuid(euuid);
  ...
  if (window._satellite) {
    window._satellite.track('setCustomerUUID', { euuid });
  }
};
