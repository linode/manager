...
<Meta title="Components/Forms/Input Fields" component={TextField} />
...
| Label       | Avoid long labels; use succinct, short and descriptive labels (a word or two) so users can quickly scan your form.                 |
|             | Label text shouldn’t take up multiple lines.                                                                                       |
| Placeholder | Placeholder is the text that users see before they interact with a field. It should be a useful guide to the input type and format |
...
- Balance this goal with the visual benefits of fields of the same length. A somewhat outsized input that aligns with the fields above and below it might be the best choice.
...
- Consider sizing the input field to the data being entered (example the field for a street address should be wider than the field for a zip code). Balance this goal with the visual benefits of fields of the same length. A somewhat outsized input that aligns with the fields above and below it might be the best choice.
### Error
