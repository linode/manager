...
describe('Error State Component Suite', () => {
  ...
  it('should display error state with text elements', () => {
    ...
    expect(errorIcon.isDisplayed())
      .withContext(`Error icon should have been displayed`)
      .toBe(true);
    expect(errorText.getText())
      .withContext(`Incorrect error text`)
      .toBe('An error has occurred.');
    expect(errorText.getAttribute('style'))
      .withContext(`Style should be centered`)
      .toBe(`text-align: center;`);
    expect(errorText.getCSSProperty('font'))
      .withContext(`Incorrect font value(s)`)
      .toEqual(fontValues);
  });
});
