...
<Meta
  ...
  argTypes={{
    label: {
      description: 'Label displayed above the Selection Drop Down',
    },
    placeholder: {
...
export const SelectTemplate = (args, context) => {
...
<Canvas>
  <Story
    ...
    argTypes={{
      ...
      options: {
        description: 'An array of objects with label and value properties. The label property is what is displayed to the user.',
      },
      isLoading: {
        description: 'When true the Selection Drop Down will have a spinning loading indicator.',
      },
      errorText: {
        description: 'Displays a message below the Selection Drop Down. The border of the Selection Drop Down will also turn red if this is set to a non-empty string',
      },
      isMulti: {
        description: 'When true it allows the user to select multiple options',
      },
      creatable: {
        description: 'When true the Selection Drop Down component allows the user to create new items to choose from',
      },
    }}
  >
    { SelectTemplate.bind({}) }
