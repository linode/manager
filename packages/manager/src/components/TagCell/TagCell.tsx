...
export interface TagCellProps {
  ...
  /**
   * Tooltip text to display when the "Add Button" is disabled.
   */
...
export const TagCell = (props: TagCellProps) => {
  const { disabled, sx, tags, updateTags, view, disabledTooltipText } = props;
  ...
  const AddButton = (props: { panel?: boolean }) => (
    <StyledTagButton
      ...
      panel={props.panel}
      title="Add a tag"
      tooltipText={`${disabled ? disabledTooltipText : ''}`}
    >
      Add a tag
    </StyledTagButton>
  );
