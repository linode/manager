...
- The side navigation currently has 13 items in three groups.
...
export const Template = (args, context) => {
  ...
  return (
    <>
      <SideMenu open={true} desktopOpen={context.open} closeMenu={() => null} />
      <button
        onClick={() => context.setOpen(!context.open)}
        style={{ float: 'right' }}
      >
        Click to toggle side menu
      </button>
    </>
  );
};
...
<Canvas>
  <Story name="Side Menu" decorators={decorators}>
    {Template.bind({})}
  </Story>
</Canvas>
