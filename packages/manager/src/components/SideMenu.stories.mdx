...
export const Template = (args) => {
  const useStyles = makeStyles(() => ({
    menu: {
      '& > div > div > div': { paddingBottom: 8, position: 'relative' },
    },
  }));
      ...
      <Box display="flex" flexDirection="column" style={{ width: '50%' }}>
        <div>
          ...
          <button onClick={() => setDesktopOpen(!desktopOpen)}>
            Click to {desktopOpen ? 'expand' : 'collapse'} side menu
          </button>
        </div>
        <br />
        <div>
          ...
          <div style={{ marginTop: 4, marginLeft: 16 }}>
            Has no effect unless the viewport is less than 960px.
          </div>
        </div>
      </Box>
    </Box>
...
<ArgsTable story="Side Menu" sort="requiredFirst" />
