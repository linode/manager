...
export const LinodeInterfacesAvailabilityNotice = () => {
  const { data: regions } = useRegionsQuery();

  const regionsThatSupportLinodeInterfaces = regions?.filter((r) =>
    r.capabilities.includes('Linode Interfaces')
  );
  ...
  return (
    <Notice sx={{ width: 'fit-content' }} variant="warning">
      ...
      >
        Linode Interfaces are not available in the selected region.{' '}
        <TextTooltip
          ...
          tooltipText={
            <List sx={{ columns: '2 auto' }}>
              {regionsThatSupportLinodeInterfaces?.map((region) => (
