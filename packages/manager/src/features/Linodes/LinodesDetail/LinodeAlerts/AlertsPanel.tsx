...
export interface AlertsPanelHandle {
  validateFormAndGetValues: () => Promise<{
    errors: FormikErrors<Linode['alerts']>;
    values: Linode['alerts'];
  }>;
}

export const AlertsPanel = React.forwardRef<AlertsPanelHandle, Props>(
  (props, ref) => {
    const {
      error,
    ...
    React.useImperativeHandle(ref, () => ({
      validateFormAndGetValues: async () => {
        const errors: FormikErrors<Linode['alerts']> =
          await formik.validateForm();
        return { values: formik.values, errors };
      },
    }));
    ...
    const alertSections: AlertSectionProps[] = [
      ...
      },
      {
        ...
        state:
          formik.values.io === ('' as unknown) || Boolean(formik.values.io),
      ...
      },
      {
        ...
        endAdornment: 'Mb/s',
        error:
          (formik.touched.network_out
            ? formik.errors.network_out
            : undefined) || hasAPIErrorFor('alerts.network_out'),
        onStateChange: (
          e: React.ChangeEvent<HTMLInputElement>,
          checked: boolean
        ) =>
