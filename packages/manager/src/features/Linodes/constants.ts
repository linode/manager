export const PUBLIC_IP_ADDRESSES_CONFIG_INTERFACE_TOOLTIP_TEXT =
  'This Public IP Address is provisionally reserved but not assigned to the network interfaces in this configuration profile.';

export const PUBLIC_IP_ADDRESSES_LINODE_INTERFACE_NOT_ASSIGNED_TOOLTIP_TEXT =
  'This Public IP Address is provisionally reserved but not assigned to a network interface.';

export const PUBLIC_IP_ADDRESSES_LINODE_INTERFACE_DEFAULT_ROUTE_TOOLTIP_TEXT =
  'This Public IP Address is provisionally reserved but not the default route. To update this, please review your Interface Settings.';

export const METRICS_LEGACY_MODE_BETA_PHASE_BANNER_TEXT =
  'Try the new Metrics (Beta) with more options and greater flexibility for better data analysis. You can switch back to the current view at any time.';

export const METRICS_LEGACY_MODE_NEW_PHASE_BANNER_TEXT =
  'Try the new Metrics with more options and greater flexibility for better data analysis. You can switch back to the current view at any time.';

export const METRICS_ACLP_MODE_BETA_PHASE_BANNER_TEXT =
  'Welcome to Metrics (Beta) with more options and greater flexibility for better data analysis.';

export const METRICS_ACLP_MODE_NEW_PHASE_BANNER_TEXT =
  'Welcome to Metrics (New) with more options and greater flexibility for better data analysis.';

export const METRICS_LEGACY_MODE_BETA_PHASE_BUTTON_TEXT =
  'Try the Metrics (Beta)';

export const METRICS_LEGACY_MODE_NEW_PHASE_BUTTON_TEXT =
  'Try the Metrics (New)';

export const METRICS_ACLP_MODE_BETA_AND_NEW_PHASE_BUTTON_TEXT =
  'Switch to legacy Metrics';

export const ALERTS_LEGACY_MODE_BETA_PHASE_BANNER_TEXT =
  'Try the Alerts (Beta), featuring new options like customizable alerts. You can switch back to legacy Alerts at any time.';

export const ALERTS_LEGACY_MODE_NEW_PHASE_BANNER_TEXT =
  'Try the Alerts (New), featuring new options like customizable alerts. You can switch back to legacy Alerts at any time.';

export const ALERTS_ACLP_MODE_BETA_PHASE_BANNER_TEXT =
  'Welcome to Alerts (Beta), designed for flexibility with features like customizable alerts.';

export const ALERTS_ACLP_MODE_NEW_PHASE_BANNER_TEXT =
  'Welcome to Alerts (New), designed for flexibility with features like customizable alerts.';

export const ALERTS_LEGACY_MODE_BETA_PHASE_BUTTON_TEXT = 'Try Alerts (Beta)';

export const ALERTS_LEGACY_MODE_NEW_PHASE_BUTTON_TEXT = 'Try Alerts (New)';

export const ALERTS_ACLP_MODE_BETA_AND_NEW_PHASE_BUTTON_TEXT =
  'Switch to legacy Alerts';

export const ALERTS_LEGACY_PROMPT =
  'Are you sure you want to save legacy Alerts?';

export const ALERTS_BETA_PROMPT =
  'Are you sure you want to save (Beta) Alerts?';

// Lock-related constants
export const LINODE_LOCKED_DELETE_TOOLTIP =
  "You don't have permission to delete this Linode.";

export const LINODE_LOCKED_DELETE_CONFIG_TOOLTIP =
  "You don't have permission to delete this configuration.";

export const LINODE_LOCKED_DELETE_DISK_TOOLTIP =
  "You don't have permission to delete this disk.";

export const LINODE_LOCKED_DELETE_IP_TOOLTIP =
  "You don't have permission to delete this IP Address.";

export const LINODE_LOCKED_DELETE_INTERFACE_TOOLTIP =
  "You don't have permission to delete this interface.";

export const LINODE_REBUILD_LOCKED_NOTICE_TEXT =
  'This Linode is currently locked and cannot be rebuilt. Please remove the lock to proceed.';

export const WARNING_MESSAGE_FOR_NO_FIREWALL_OPTION =
  'This Linode, or its Linode interface, is not secured with a Cloud Firewall. Add a firewall to help protect your resources and simplify security management.';
