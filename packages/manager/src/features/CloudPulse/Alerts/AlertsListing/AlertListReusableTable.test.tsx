...
import { AlertListReusableTable, type TableColumnHeader } from './AlertListReusableTable';

import type { AlertListReusableTableProps} from './AlertListReusableTable';

const serviceType = 'linode';
const entityId = '123';
const entityName = 'test-instance';
const alerts = [...alertFactory.buildList(7, {entity_ids: [entityId], service_type: serviceType, status:"enabled"})];
...
describe('Alert Listing Reusable Table for contextual view', () => {
  ...
  it("Should render alert table", () =>{
      const {getByText} = renderWithTheme(<AlertListReusableTable {...props} />)
  ...
  it("Should show message for empty table", ()=>{
    const { getByText} = renderWithTheme(<AlertListReusableTable {...props} alerts={[]} />)

    expect(getByText("No data to display.")).toBeInTheDocument();
  })

  it("Shoud render table row toggle in table row", async ()=>{
    const { findByTestId} = renderWithTheme(<AlertListReusableTable {...props}/>)
  ...
  it("Should show confirm dialog on checkbox click", async ()=>{
    const {findByTestId, findByText} = renderWithTheme(<AlertListReusableTable {...props} />)
    const alert=alerts[0];
    const row = await findByTestId(alert.id);
