...
const getTotalMemoryUsage = (used: number, free: number) => {
...
const sumStorage = (DiskData: Record<string, Disk> = {}): Storage => {
  let free = 0;
  let total = 0;
...
const sumCPU = (CPUData: Record<string, CPU> = {}): CPU<'yAsNull'> => {
 ...
 */
const sumUsedMemory = (data: LVClientData) => {
...
const statAverage = (stats: Stat[] = []): number => {
  if (stats.length === 0) {
    return 0;
  }
...
const statMax = (stats: StatWithDummyPoint[] = []): number => {
  if (stats.length === 0) {
    return 0;
  }
 ...
 */
const sumStatsObject = <T extends {}>(
 ...
 */
const sumRelatedProcessesAcrossAllUsers = (
...
const getMaxUnitAndFormatNetwork = (
