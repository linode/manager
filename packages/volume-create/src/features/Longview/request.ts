...
const fieldNames: Record<LongviewFieldName, string> = {
...
const getLastUpdated = (token: string) => {
  return get(token, 'lastUpdated');
};

const getValues = curry((token: string, options: Options) => {
  return get(token, 'getValues', options);
});

const getLatestValue = curry((token: string, options: Options) => {
  return get(token, 'getLatestValue', options);
});

const getTopProcesses = curry((token: string, options?: Options) => {
