...
type StackScriptCategory = 'account' | 'community';

const emptyResult: ResourcePage<StackScript> = {
  data: [],
  page: 1,
  pages: 1,
  results: 0,
...
const getStackScriptsByUser: StackScriptsRequest = (
  username: string,
  params?: Params,
  filter?: Filter
) =>
...
const getMineAndAccountStackScripts: StackScriptsRequest = (
  params?: Params,
  filter?: Filter
) => {
 ...
 */
const getCommunityStackscripts: StackScriptsRequest = (
  params?: Params,
  filter?: Filter
) => {
...
type AcceptedFilters = 'description' | 'label' | 'username';
...
const canUserModifyAccountStackScript = (
