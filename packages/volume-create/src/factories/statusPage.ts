...
const pageFactory = Factory.Sync.makeFactory<IncidentPage>({
...
const incidentUpdateFactory = Factory.Sync.makeFactory<IncidentUpdate>({
  affected_components: [
    {
      code: v4(),
      name:
...
const incidentFactory = Factory.Sync.makeFactory<Incident>({
  created_at: DATE,
  id: Factory.each((i) => String(i)),
  impact: 'critical',
...
const maintenanceFactory = Factory.Sync.makeFactory<Maintenance>({
  created_at: DATE,
  id: Factory.each((i) => String(i)),
