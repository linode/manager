...
const linodeAlertsFactory = Factory.Sync.makeFactory<LinodeAlerts>({
  cpu: 10,
  io: 10000,
  network_in: 0,
  network_out: 0,
...
const linodeSpecsFactory = Factory.Sync.makeFactory<LinodeSpecs>({
...
const generateLinodeStatSeries = (): [number, number][] => {
  const stat = [];
  let i = 0;
  for (i; i < 300; i++) {
...
const linodeNetStatsFactory = Factory.Sync.makeFactory<NetStats>({
...
const statsDataFactory = Factory.Sync.makeFactory<StatsData>({
...
const linodeBackupsFactory = Factory.Sync.makeFactory<LinodeBackups>({
  enabled: true,
...
const linodePlacementGroupPayloadFactory = Factory.Sync.makeFactory<LinodePlacementGroupPayload>(
  {
    id: Factory.each((i) => i),
...
const createLinodeRequestFactory = Factory.Sync.makeFactory<CreateLinodeRequest>(
  {
    booted: true,
    image: 'linode/debian10',
    label: Factory.each((i) => `linode-${i}`),
...
const backupFactory = Factory.Sync.makeFactory<LinodeBackup>({
  available: true,
