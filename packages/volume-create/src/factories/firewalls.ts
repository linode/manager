...
const firewallRuleFactory = Factory.Sync.makeFactory<FirewallRuleType>({
  action: 'DROP',
  addresses: {
    ipv4: ['0.0.0.0/0'],
    ipv6: ['::/0'],
...
const firewallRulesFactory = Factory.Sync.makeFactory<FirewallRules>({
...
const firewallTemplateFactory = Factory.Sync.makeFactory<FirewallTemplate>(
  {
    rules: firewallRulesFactory.build(),
