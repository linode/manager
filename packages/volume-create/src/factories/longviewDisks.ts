...
const diskFactory = Factory.Sync.makeFactory<Disk>({
  childof: 0,
  children: 0,
  dm: 0,
  isswap: 0,
...
const cpuFactory = Factory.Sync.makeFactory<CPU>({
  system: [mockStats[7]],
  wait: [mockStats[8]],
  user: [mockStats[9]],
});
...
const InboundOutboundNetworkFactory = Factory.Sync.makeFactory<InboundOutboundNetwork>(
  {
...
const LongviewNetworkInterfaceFactory = Factory.Sync.makeFactory<LongviewNetworkInterface>(
  {
