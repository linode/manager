...
const kubeLinodeFactory = Factory.Sync.makeFactory<PoolNodeResponse>({
...
const kubernetesClusterFactory = Factory.Sync.makeFactory<KubernetesCluster>(
  {
...
const kubernetesDashboardUrlFactory = Factory.Sync.makeFactory<KubernetesDashboardResponse>(
  {
    url: `https://${v4()}`,
  }
);
...
const kubernetesStandardTierVersionFactory = Factory.Sync.makeFactory<KubernetesTieredVersion>(
  {
...
const kubernetesEnterpriseTierVersionFactory = Factory.Sync.makeFactory<KubernetesTieredVersion>(
  {
...
const kubernetesControlPlaneACLOptionsFactory = Factory.Sync.makeFactory<ControlPlaneACLOptions>(
  {
    addresses: {
...
const kubernetesControlPlaneACLFactory = Factory.Sync.makeFactory<KubernetesControlPlaneACLPayload>(
  {
    acl: {
